...
uniform vec3 Cub;			
uniform vec3 ColCub1;			
uniform vec3 ColCub2;			
uniform vec3 ColCub3;			
uniform vec3 Tet;				
uniform vec3 ColTet1;				
uniform vec3 ColTet2;			
...
void initializeDefaultLightMaterials(out SLight light)
{
	...
	materials[0].Color = ColSmallSphere;			
	...
	materials[7].Color = ColBigSphere;					
...
void initializeDefaultScene()
{
	...
	if (BigSphere != vec3(0.0, 0.0, 0.0))			
	{
		spheres[0].Center = vec3(-1.0,-2.0,-2.0);
		spheres[0].Radius = 2.0;
		spheres[0].MaterialIdx = 0;
	}
	if (SmallSphere != vec3(0.0, 0.0, 0.0))			
	{
		spheres[1].Center = vec3(2.0,1.0,2.0);
		spheres[1].Radius = 1.0;
		spheres[1].MaterialIdx = 0;
	}
	if (Cub != vec3(0.0, 0.0, 0.0))					
	{
	/** Kubs **/
	kubs[0].v1 = vec3(-1.5,-1.5,-1.0);	
	kubs[0].v2 = vec3(-1.0, 1.0, 1.0);	
	kubs[0].v3 = vec3(-1.5, 0.5,-1.0);	
	kubs[0].MaterialIdx = 0;
	kubs[1].v1 = vec3(-1.5,-1.5,-1.0);	
	kubs[1].v2 = vec3(-1.0,-1.0, 1.0);	
	kubs[1].v3 = vec3(-1.0, 1.0, 1.0);							
	kubs[1].MaterialIdx = 0;
	/* back wall */
	kubs[2].v1 = vec3(-1.0,-1.0, 1.0);	
	kubs[2].v2 = vec3( 1.0,-1.0, 1.0);	
	kubs[2].v3 = vec3(-1.0, 1.0, 1.0);	
	kubs[2].MaterialIdx = 0;
	kubs[3].v1 = vec3( 1.0, 1.0, 1.0);	
	kubs[3].v2 = vec3(-1.0, 1.0, 1.0);	
	kubs[3].v3 = vec3( 1.0,-1.0, 1.0);	
	kubs[3].MaterialIdx = 0;
	/*right wall */ 
	kubs[4].v1 = vec3(1.0, 1.0, 1.0);	
	kubs[4].v2 = vec3(1.0, -1.0, 1.0);	
	kubs[4].v3 = vec3(0.5, 0.5, -1.0);	
	kubs[4].MaterialIdx = 0; 
	kubs[5].v1 = vec3(0.5, 0.5, -1.0);	
	kubs[5].v2 = vec3(1.0, -1.0, 1.0);	
	kubs[5].v3 = vec3(0.5, -1.5, -1.0); 
	kubs[5].MaterialIdx = 0; 
	/*down wall */ 
	kubs[6].v1 = vec3(-1.0,-1.0, 1.0); 
	kubs[6].v2 = vec3(-1.5,-1.5,-1.0); 
	kubs[6].v3 = vec3( 1.0,-1.0, 1.0); 
	kubs[6].MaterialIdx = 0; 
	kubs[7].v1 = vec3(0.5, -1.5, -1.0);
	kubs[7].v2 = vec3(1.0,-1.0, 1.0);	
	kubs[7].v3 = vec3(-1.5,-1.5,-1.0);	
	kubs[7].MaterialIdx = 0; 
	/*up wall */ 
	kubs[8].v1 = vec3(-1.5, 0.5,-1.0); 
	kubs[8].v2 = vec3(-1.0, 1.0, 1.0); 
	kubs[8].v3 = vec3( 1.0, 1.0, 1.0); 
	kubs[8].MaterialIdx = 0; 
	kubs[9].v1 = vec3(-1.5, 0.5, -1.0);
	kubs[9].v2 = vec3( 1.0, 1.0, 1.0);	
	kubs[9].v3 = vec3(0.5, 0.5, -1.0);	
	kubs[9].MaterialIdx = 0; 

	kubs[10].v1 = vec3(-1.5,-1.5, -1.0);
	kubs[10].v2 = vec3( 0.5,-1.5, -1.0);
	kubs[10].v3 = vec3(-1.5, 0.5, -1.0);
	kubs[10].MaterialIdx = 0; 
	kubs[11].v1 = vec3( 0.5, 0.5, -1.0);
	kubs[11].v2 = vec3(-1.5, 0.5, -1.0);
	kubs[11].v3 = vec3( 0.5,-1.5, -1.0);
	kubs[11].MaterialIdx = 0;
	}

	if (Tet != vec3(0.0, 0.0, 0.0))				
	{
	/** Thetr **/
	
	thetrs[0].v1 = vec3(0.0,-1.0, -3.5); 
	thetrs[0].v2 = vec3( 1.5,-1.5, -5.0);
	thetrs[0].v3 = vec3(3.0,-1.0, -3.5); 
	thetrs[0].MaterialIdx = 0;
	thetrs[1].v1 = vec3(0.0,-1.0, -3.5); 
	thetrs[1].v2 = vec3( 1.5,-1.5, -5.0);
	thetrs[1].v3 = vec3( 1.3, 0.5, -5.0);
	thetrs[1].MaterialIdx = 0;
	
	thetrs[2].v1 = vec3( 1.5,-1.5, -5.0);
	thetrs[2].v2 = vec3(3.0,-1.0, -3.5); 
	thetrs[2].v3 = vec3( 1.3, 0.5, -5.0);
	thetrs[2].MaterialIdx = 0;
	thetrs[3].v1 = vec3(0.0,-1.0, -3.5); 
	thetrs[3].v2 = vec3(3.0,-1.0, -3.5); 
	thetrs[3].v3 = vec3( 1.3, 0.5, -5.0);
...
bool Raytrace ( SRay ray, float start, float final, inout SIntersection intersect )
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if( IntersectSphere (sphere, ray, start, final, test ) && test < intersect.Time )
		{
			...
			if ((i == 1) && (SmallSphere != vec3(0.0, 0.0, 0.0)))		
			{
			...
			if ((i == 0) && (BigSphere != vec3(0.0, 0.0, 0.0)))		
			{
