...
void initializeDefaultLightMaterials(out SLight light)
{
	...
	//"общая коробка"
	materials[1].Color = vec3(0.0, 1.0, 0.0);	//левая грань
	materials[1].LightCoeffs = vec4(lightCoefs);
	materials[1].ReflectionCoef = 0.5;
	materials[1].RefractionCoef = 1.0;
	materials[1].MaterialType = MIRROR_REFLECTION;	
	
	materials[2].Color = vec3(1.0, 0.0, 0.0);	//задняя грань
	...
	materials[3].Color = vec3(0.5, 0.5, 0.0);	//правая грань
	...
	materials[4].Color = vec3(0.5, 0.5, 0.5);	//нижняя грань
	...
	materials[5].Color = vec3(1.0, 1.0, 1.0);	//верхняя грань
